...
def render_tracker():
    ...
    # Function to generate the HTML for a single compact card
    def generate_card_html(label, rows_list, css_class):
        if not rows_list:
            return f"""
            <div class="tracker-card {css_class}">
                <div class="tc-label">{label}</div>
                <div class="tc-time">---</div>
                <div class="tc-details" style="color: #adb5bd;">No Class Scheduled</div>
            </div>
            """
        ...
        <div class="tracker-card {css_class}">
            <div class="tc-label">{label}</div>
            <div class="tc-time">{time_str}</div>
            <div class="tc-details">{details}</div>
        </div>
        """

    # Injecting the CSS and HTML for the 3-Card Layout
    html_content = f"""
    <style>
        .tracker-container {{
            display: flex;
        ...
        /* Vibrant Green for Ongoing Classes */
        .card-current {{ 
        ...
        /* Clean Neutral for Future Classes */
        .card-future {{ 
