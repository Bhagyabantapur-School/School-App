...
# Time & Date Logic
now = datetime.now()
curr_date_str = now.strftime("%d-%m-%Y") # Format matches your Leave CSV
curr_time = now.time()
...
# ==========================================
    ...
    # Dropdown with "Select..." placeholder
    ...
    if t_name_select != "Select Your Name...":
        ...
        if t_pw == TEACHER_CREDS.get(t_name_select):
            # Holiday Check
            h_df = get_csv('holidays.csv')
            # Check date format in holidays.csv (assuming DD-MM-YYYY)
            ...
            if is_h or now.strftime('%A') == 'Sunday':
                        ...
                        if sel_class != "Select Class...":
                            ...
                            if not students.empty:
                                ...
                                if roster.empty:
                                    ...
                                    # (Optional: Add logic here to match QR val to student)
                ...
                # --- LEAVE STATUS ---
                with at_tabs[2]:
                    st.subheader("Casual Leave (CL) Record")
                    # Read the uploaded teacher_leave.csv
                    ...
                    if not leave_log.empty:
                        ...
                        st.metric("Leaves Remaining (Yearly Limit: 14)", f"{14 - taken}")
                        st.write("Leave History:")
                        st.dataframe(my_leaves, hide_index=True)
                    else:
                        st.metric("Leaves Remaining", "14")
...
# ==========================================
    ...
    if admin_pw == "bpsAPP@2026":
        tabs = st.tabs(["📊 Summary", "📝 Attendance", "👨‍🏫 Substitution", "👟 Shoes", "🆔 Cards", "📢 Notice"])
        ...
        with tabs[0]: 
            ...
            if not att_master.empty:
                ...
                # Filter for Present (Status=True)
                ...
                if not students.empty:
                    ...
                    if not roster.empty:
                        ...
                        if st.button(f"Save {sel_c} Attendance"):
                            # Filter for date to avoid duplicates (optional logic)
                            recs = ed_att.copy()
                            # Format for CSV
                            ...
                            st.success("Saved!")
                    else: st.warning("No students in this class.")
        ...
        with tabs[2]:
            ...
            leave_log = get_csv('teacher_leave.csv')
            if not leave_log.empty:
                today_subs = leave_log[leave_log['Date'] == curr_date_str]
                if not today_subs.empty:
                    st.write("Today's Substitutions:")
                    st.dataframe(today_subs, hide_index=True)
            
            st.divider()
            st.write("Mark New Absence (Mid-day or Planned)")
            ...
            if abs_t != "Select...":
                ...
                if st.button("Confirm Absence"):
                    ...
                    st.success(f"Absence recorded for {abs_t}")
        ...
        with tabs[3]:
            ...
            if s_class != "Select Class...":
                ...
                if not students.empty:
                    ...
                    ed_shoe = st.data_editor(
                        roster[['Roll', 'Name', 'Received_Before', 'Mark_Received', 'Remark']],
                        disabled=['Roll', 'Name', 'Received_Before'],
                        hide_index=True, use_container_width=True
                    )
                    
                    if st.button("Update Shoe Records"):
                        # Save only new 'Mark_Received'
