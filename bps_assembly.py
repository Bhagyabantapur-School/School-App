...
if st.session_state.get('user_role') != "admin":
    st.error("🚫 Access Denied: This Planner is strictly reserved for the Head Teacher (Admin).")
    st.stop()

def inject_app_css(user_name):
    wm = f"{user_name} - HEAD TEACHER NOTEBOOK"
    st.markdown(f"""<style>
        ...
        .meet-card {{ background: linear-gradient(135deg, #ffffff, #fcf8e3); border: 3px solid #f0ad4e; border-radius: 18px; padding: 25px; margin-bottom: 20px; box-shadow: 0px 8px 20px rgba(0,0,0,0.08); }}
        ...
        .meet-title {{ color: #d58512; font-size: 24px; font-weight: 900; margin-bottom: 10px; border-bottom: 2px solid #faebcc; padding-bottom: 8px; }}
        .cue-point {{ font-size: 18px; line-height: 1.5; color: #212529; margin-bottom: 10px; padding-left: 10px; border-left: 4px solid #28a745; }}
        .meet-point {{ font-size: 18px; line-height: 1.5; color: #212529; margin-bottom: 10px; padding-left: 10px; border-left: 4px solid #d9534f; }}
...
inject_app_css(st.session_state.user_name)
...
# --- Teacher Meetings Data Handlers ---
@st.cache_data(ttl=300)
def fetch_meeting_logs():
    sh = init_assembly_sheet()
    ws = ensure_worksheet(sh, "teacher_meetings", ["Date", "Day", "Main_Agenda", "Discussion_Points", "Pending_Tasks"])
    df = pd.DataFrame(ws.get_all_records()).astype(str)
    df.columns = [str(c).strip() for c in df.columns]
    return df

def save_meeting_log(record_dict):
    sh = init_assembly_sheet()
    ws = ensure_worksheet(sh, "teacher_meetings", ["Date", "Day", "Main_Agenda", "Discussion_Points", "Pending_Tasks"])
    records = ws.get_all_records()
    df = pd.DataFrame(records).astype(str)
    ...
    ws.clear()
    ws.update([new_df.columns.values.tolist()] + new_df.fillna("").values.tolist())
    fetch_meeting_logs.clear()

def delete_meeting_log(date_str):
    sh = init_assembly_sheet()
    ws = ensure_worksheet(sh, "teacher_meetings", ["Date", "Day", "Main_Agenda", "Discussion_Points", "Pending_Tasks"])
    records = ws.get_all_records()
    df = pd.DataFrame(records).astype(str)
    ...
    if not df.empty and 'Date' in df.columns:
        ...
        if not filtered_df.empty:
            ...
            ws.append_row(["Date", "Day", "Main_Agenda", "Discussion_Points", "Pending_Tasks"])
    fetch_meeting_logs.clear()
...
# ==========================================
# 4. MAIN USER INTERFACE & NAVIGATION
# ==========================================
st.sidebar.markdown("### 📌 Planning Modules")
app_mode = st.sidebar.radio("Select Module:", ["🎙️ Morning Assembly", "👨‍🏫 Teacher Meetings"])
st.sidebar.divider()
st.sidebar.button("🔄 Sync Records", on_click=lambda: (fetch_assembly_logs.clear(), fetch_meeting_logs.clear()), use_container_width=True)

if app_mode == "🎙️ Morning Assembly":
    st.markdown("<h2>🎙️ Morning Assembly Planner</h2>", unsafe_allow_html=True)
...
elif app_mode == "👨‍🏫 Teacher Meetings":
    st.markdown("<h2>👨‍🏫 Teacher Meeting Planner</h2>", unsafe_allow_html=True)
    st.caption("Jot down topics to discuss with assistant teachers and track pending tasks.")

    meet_tabs = st.tabs(["📝 Plan Meeting Agenda", "📱 Live Meeting Agenda", "🗄️ Meeting Archive"])

    meet_logs_df = fetch_meeting_logs()
    today_meet_log = meet_logs_df[meet_logs_df['Date'].astype(str).str.strip() == curr_date_str] if not meet_logs_df.empty and 'Date' in meet_logs_df.columns else pd.DataFrame()

    with meet_tabs[0]:
        st.markdown("#### 📋 Draft Meeting Notes")
        
        default_agenda = ""
        default_discuss = ""
        default_tasks = ""
        
        if not today_meet_log.empty:
            default_agenda = today_meet_log.iloc[0].get('Main_Agenda', '')
            default_discuss = today_meet_log.iloc[0].get('Discussion_Points', '').replace(' || ', '\n')
            default_tasks = today_meet_log.iloc[0].get('Pending_Tasks', '').replace(' || ', '\n')

        with st.form("meeting_plan_form"):
            c1, c2 = st.columns([1, 2])
            meet_date = c1.date_input("Meeting Date", datetime.now()).strftime("%d-%m-%Y")
            agenda_input = c2.text_input("Main Agenda / Purpose", value=default_agenda, placeholder="e.g., Evaluation-II Planning & MDM Duty")
            
            st.markdown("##### 🗣️ Topics to Discuss (1 Topic Per Line)")
            discuss_input = st.text_area(
                "Write down everything you need to remember to tell the teachers:",
                value=default_discuss,
                height=130,
                placeholder="1. Verify exam subject assignments in the portal.\n2. Submit pending exam fees by Friday.\n3. Keep classrooms locked during Tiffin."
            )
            
            st.markdown("##### 📌 Pending Tasks / Follow-ups (Optional)")
            tasks_input = st.text_area(
                "Tasks you assigned or need to follow up on:",
                value=default_tasks,
                height=100,
                placeholder="1. Susmita Madam to order new chalks.\n2. Bimal Sir to check sports equipment."
            )
            
            if st.form_submit_button("💾 Save Meeting Agenda", type="primary"):
                if not agenda_input:
                    st.error("Please enter a Main Agenda for the meeting.")
                else:
                    formatted_discuss = " || ".join([p.strip() for p in discuss_input.split('\n') if p.strip()])
                    formatted_tasks = " || ".join([p.strip() for p in tasks_input.split('\n') if p.strip()])
                    record = {
                        "Date": meet_date,
                        "Day": datetime.strptime(meet_date, "%d-%m-%Y").strftime("%A"),
                        "Main_Agenda": agenda_input.strip(),
                        "Discussion_Points": formatted_discuss,
                        "Pending_Tasks": formatted_tasks
                    }
                    save_meeting_log(record)
                    st.success(f"✅ Meeting agenda saved for {meet_date}!")
                    st.rerun()

    with meet_tabs[1]:
        st.markdown("#### 📱 Live Meeting View")
        st.caption("A clean view to look at while conducting the meeting.")
        
        view_meet_date = st.date_input("Select Meeting Date", datetime.now(), key="cue_meet_date").strftime("%d-%m-%Y")
        card_meet_log = meet_logs_df[meet_logs_df['Date'].astype(str).str.strip() == view_meet_date] if not meet_logs_df.empty and 'Date' in meet_logs_df.columns else pd.DataFrame()
        
        if card_meet_log.empty:
            st.warning(f"No meeting agenda found for **{view_meet_date}**. Please draft one in the first tab!")
        else:
            data = card_meet_log.iloc[0]
            agenda_str = data.get('Main_Agenda', 'Staff Meeting')
            discuss_list = [p.strip() for p in str(data.get('Discussion_Points', '')).split(' || ') if p.strip()]
            tasks_list = [p.strip() for p in str(data.get('Pending_Tasks', '')).split(' || ') if p.strip()]
            
            st.markdown(f"""
            <div class="meet-card">
                <div class="meet-title">👥 {agenda_str}</div>
                <p style="color:gray; font-weight:bold; margin-top:-5px; margin-bottom:15px;">📅 {view_meet_date} ({data.get('Day', '')})</p>
            </div>
            """, unsafe_allow_html=True)
            
            st.markdown("### 🗣️ Discussion Points")
            if discuss_list:
                for pt in discuss_list:
                    st.markdown(f'<div class="meet-point">🔹 {pt}</div>', unsafe_allow_html=True)
            else:
                st.info("No discussion points entered.")
                
            if tasks_list:
                st.markdown("### 📌 Action Items & Tasks")
                for task in tasks_list:
                    st.markdown(f'<div class="cue-notice">☑️ {task}</div>', unsafe_allow_html=True)

    with meet_tabs[2]:
        st.markdown("#### 🗄️ History of Teacher Meetings")
        
        if not meet_logs_df.empty and 'Date' in meet_logs_df.columns:
            display_meet_df = meet_logs_df.copy()
            display_meet_df['Discussion_Points'] = display_meet_df['Discussion_Points'].str.replace(' || ', ' • ')
            display_meet_df['Pending_Tasks'] = display_meet_df['Pending_Tasks'].str.replace(' || ', ' • ')
            st.dataframe(
                display_meet_df[['Date', 'Day', 'Main_Agenda', 'Discussion_Points', 'Pending_Tasks']],
                hide_index=True,
                use_container_width=True
            )
            
            st.markdown("---")
            del_meet_date = st.selectbox("Select Date to Delete Record", ["Select..."] + display_meet_df['Date'].tolist(), key="del_meet")
            if del_meet_date != "Select..." and st.button("🗑️ Delete Selected Meeting Record"):
                delete_meeting_log(del_meet_date)
                st.success("Deleted from database!")
                st.rerun()
        else:
            st.info("No past meeting records stored yet.")
