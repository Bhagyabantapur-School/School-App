    ...
    with st.form("start_task_form"):
        ...
        if video_phase == "Publishing (YT)":
            ...
            if transfer_selection.startswith("➕ Add New"):
                recording_device = st.text_input("Enter New Transfer Path (e.g., SD Card to PC W10)")
            else:
                recording_device = transfer_selection
        ...
        if st.form_submit_button("Start Phase"):
            if not event_name:
                ...
                st.error("Please enter a Video Title.")
            elif video_phase == "Transferring" and transfer_selection.startswith("➕ Add New") and not recording_device:
                st.error("Please enter the new Transfer Path.")
            elif video_phase not in ["Publishing (YT)", "Transferring"] and soft_selection.startswith("➕ Add New") and not editing_tool:
                st.error(f"Please enter the name of the new {term.lower()}.")
            else:
                now = get_ist_time()
