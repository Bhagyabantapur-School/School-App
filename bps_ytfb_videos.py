...
# ------------------------------------------
# TAB 1: START A PHASE (With Conditional YT Fields)
# ------------------------------------------
with tab1:
    with st.form("start_task_form"):
        ...
        # --- CONDITIONAL LOGIC FOR YT PUBLISHING ---
        if video_phase == "Publishing (YT)":
            ...
            # Hidden defaults for YT phase
            recording_device = "N/A"
            editing_tool = "N/A"
            # Special Metadata Payload for YT
        ...
        if start_button:
            if not event_name:
                ...
                st.success(f"Started '{video_phase}' at {start_time_str}!")
