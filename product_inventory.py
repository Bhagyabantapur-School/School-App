...
# Back to Hub
if st.button("⬅️ Back to Hub"): st.switch_page("routine_app.py")
...
with st.expander("➕ Add New Product", expanded=True):
    ...
    with col2:
        # Changed to text_input to handle "MM/YY" or exact dates flexibly
        mfd = st.text_input("MFD (e.g., 10/25 or DD-MM-YY)")
        exp = st.text_input("EXP (e.g., 12/26 or DD-MM-YY)")
        
        # --- DYNAMIC SIZE DROPDOWN LOGIC ---
        existing_sizes = []
    ...
    if st.button("💾 Save Product", type="primary"):
        if p_name and size:
            ...
            load_inventory.clear() # Clear cache so the new size appears in the dropdown next time
            ...
            with st.container(border=True):
                ...
                with c3:
                    if st.button("✅ Finish", key=f"fin_{idx}"):
                        ...
                        load_inventory.clear() # Clear cache so the finished product disappears from view
                        st.rerun()
else:
