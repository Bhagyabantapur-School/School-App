...
# 1. Initialize session state to hold our processed images
if 'resized_images' not in st.session_state:
    st.session_state.resized_images = []

target_size = (150, 150)

# 2. Upload the original files
...
if uploaded_files:
    ...
    if st.button("Step 2: Resize Photos"):
        # Clear the memory for any previous uploads
        st.session_state.resized_images = [] 
...
# 4. Show download buttons if there are resized images ready
