...
try:
    ...
    tab1, tab2, tab3 = st.tabs(["⏱️ Live", "💧 Water", "📍 Places"])
    ...
    with tab1:
        ...
        if 4.5 <= current_hour_dec <= 20.5: 
            if not today_water.empty:
                ...
                try:
                    last_water_dt = ist_timezone.localize(datetime.strptime(f"{today_str} {last_water_time_str}", "%Y-%m-%d %H:%M"))
                    hours_since_water = (now - last_water_dt).total_seconds() / 3600
                    if hours_since_water >= 2.0: st.warning(f"💧 **Hydration Reminder:** It's been {int(hours_since_water)} hours since your last drink. Time for water!")
                except: pass
            else: st.warning("💧 **Hydration Reminder:** You haven't logged any water yet today!")
        ...
        with col_w1:
            if st.button("🥃 250 ml", use_container_width=True):
                smart_append_row(get_sheet("water_log"), [today_str, now.strftime('%H:%M'), 250])
                get_water_log.clear() 
                st.rerun()
        with col_w2:
            if st.button("🚰 500 ml", use_container_width=True):
                smart_append_row(get_sheet("water_log"), [today_str, now.strftime('%H:%M'), 500])
                get_water_log.clear() 
                st.rerun()
        with col_w3:
            if st.button("🥛 1000 ml", use_container_width=True):
                smart_append_row(get_sheet("water_log"), [today_str, now.strftime('%H:%M'), 1000])
                get_water_log.clear() 
                st.rerun()
    ...
    with tab3:
        ...
        if st.button("🔄 Generate & Sync to Google Sheets", type="primary", use_container_width=True):
            with st.spinner("Analyzing location history and syncing..."):
                ...
                if valid_visits:
                    grouped = pd.DataFrame(valid_visits).groupby(['Place', 'Purpose'])['Visit_DateTime'].apply(lambda x: '\n'.join(list(dict.fromkeys(x)))).reset_index()
                    try:
                        ss = init_connection().open("sk_money_location")
                        ...
                        ws.clear()
                        ws.update(values=[["Place", "Purpose", "Visit Dates & Times"]] + grouped.values.tolist(), range_name="A1")
                        st.success("✅ 'VISITED_PLACES' tab successfully updated!")
                    except Exception as e: st.error(f"Failed to write: {e}")
                else: st.info("No stationary places found to log.")
