...
try:
    ...
    app_groups = {
        ...
        "ROUTINE": [("Live Routine Hub", "routine_app.py", "⏱️"), ("Routine Audit", "routine_audit.py", "🔍"), ("Routine Editor", "routine_editor.py", "✏️"), ("Project App", "project_app.py", "🚀"), ("AI Video Tracker", "ai_video_tracker.py", "🎬"), ("Courses", "courses.py", "📚")],
        "HEALTH": [("Health Hub", "health_app.py", "❤️"), ("Sleep & Water", "sleep_water_app.py", "💧")],
        "SCH WORK": [("MDM Returns", "mdm_return_log.py", "📦"), ("Video Manager", "bps_ytfb_videos.py", "🎬"), ("Speech Mastery", "speech_prep_app.py", "🎙️"), ("Portal Registry", "portal_registry_app.py", "🌐"), ("Image Resizer", "resizer.py", "🖼️")],
    ...
    with tab_schedule:
        ...
        prev_rows = today_schedule[start_idx : end_idx]
        ...
        # NEXT 10 ACTIVITIES (BLUE COLOR)
        next_10_rows = today_schedule[next_start_index : next_start_index+10]
        if next_10_rows:
            for n_row in next_10_rows:
                n_act = str(n_row['Activity']).strip().upper()
                n_sub = str(n_row.get('Sub_Activities', '')).strip()
                if not n_sub: n_sub = "Routine Tasks"
                n_time = f"{n_row['Start_Time']} - {n_row['End_Time']}"
