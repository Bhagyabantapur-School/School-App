...
with st.form("solar_proposal_form"):
    ...
    if submit_btn:
        if not udise_code or not school_name_loc or not roof_space:
            ...
            # Check for duplicates based on UDISE
            if not existing_data.empty and 'UDISE Code' in existing_data.columns:
                if int(udise_code) in existing_data['UDISE Code'].astype(int).values:
                    st.warning(f"⚠️ A proposal for UDISE {udise_code} has already been submitted.")
                    st.stop()
            
            # Calculate next Serial Number
            if not existing_data.empty and 'Sl. No.' in existing_data.columns:
            ...
            try:
                ...
                fetch_existing_data.clear() # Clear cache to show new data
                st.success(f"🎉 Proposal for {school_name_loc} submitted successfully!")
                st.balloons()
            except Exception as e:
                st.error(f"⚠️ Failed to save to Google Sheets: {e}")
