...
def get_google_credentials():
    ...
    scopes = [
        ...
        "https://www.googleapis.com/auth/drive.readonly" # Explicitly requesting read access for Drive
    ]
...
@st.cache_data(ttl=3600, show_spinner=False) # Cache images for 1 hour to load faster
def fetch_secure_image_bytes(file_id):
    """Securely downloads the image using the authorized service account."""
    try:
        creds = get_google_credentials()
        authed_session = AuthorizedSession(creds)
        # Drive API v3 endpoint for downloading file content
...
def display_student_photo(url):
    """Extracts the ID and passes it to the secure fetcher, handling rendering."""
    ...
    file_id = None
    # Extract the file ID from a standard Google Drive sharing link
    ...
    if file_id:
        ...
        # If it's a regular web link (not drive), try to display it normally
        try:
        ...
        if selected_name:
            ...
            if not student_record.empty:
                ...
                with head_col1:
                    ...
                    # Call the new secure display function
