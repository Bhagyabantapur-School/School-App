...
# Create dropdowns that filter based on previous selections
class_list = sorted(df_master['Class'].unique().tolist())
selected_class = st.sidebar.selectbox("1. Select Class", class_list)

if selected_class:
    filtered_by_class = df_master[df_master['Class'] == selected_class]
    section_list = sorted(filtered_by_class['Section'].unique().tolist())
    ...
    if selected_section:
        ...
        # Sort names alphabetically or by Roll
        student_list = sorted(filtered_by_section['Name'].unique().tolist())
        selected_name = st.sidebar.selectbox("3. Select Student", student_list)

        if selected_name:
            # Get the specific student's master record
            student = filtered_by_section[filtered_by_section['Name'] == selected_name].iloc[0]
            
            # --- PROFILE HEADER ---
            st.subheader(f"Profile: {student['Name']}")
            st.write(f"**Class:** {student['Class']} '{student['Section']}' | **Roll No:** {student['Roll']} | **Student Code:** {student.get('Student Code', 'N/A')}")
            ...
            with col1:
                ...
                if not student_att.empty:
                    # Convert True/False strings to booleans if necessary
            ...
            with col2:
                ...
                if not student_mdm.empty:
                    # Show the last 5 days they took MDM
